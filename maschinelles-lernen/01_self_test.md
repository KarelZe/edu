...
* _How to search for nearest neighbours efficiently?🧠_
  ...
  * Searching goes like this:
    ...
    * Update $$\boldsymbol{x}^{*}$$ if new nearest neighbour has been found.
...
* _How does the EM algorithm work?🧠_
  ...
  * The E step maximizes the lower bound with respect to some arbitrary distribution $$q(z)$$of the latent variable $$z$$, by minimizing / vanishing the Kullback Leibler distance of $$q(z)$$. The lower bound is tight, as the marginal likelihood equals the lower bound.
...
* What can you read from the loss-curves during training (validation and training loss)?🧠
  ...
  * Poor initialization, if loss decreases late.
